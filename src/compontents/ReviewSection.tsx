...
function _ReviewSection({
  ...
  const [rating, setRating] = useState(
    Number(stats?.currentUserReview?.rating || 0)
  );
  const [hoverRating, setHoverRating] = useState(0);
  const [selectedTags, setSelectedTags] = useState<string[]>(
    stats?.currentUserReview?.tags || []
  );
  const [customTag, setCustomTag] = useState('');
  const [comment, setComment] = useState(
    stats?.currentUserReview?.comment || ''
  );
  const activeCategory = stats?.isKol ? 'kol' : 'project';
  const [isAddingTag, setIsAddingTag] = useState(false);
  const [errorInfo, setErrorInfo] = useState('');
  const [, setTips] = useGlobalTips();
  const { runAsync: postHandeReviewInfoExec, loading: postReviewInfoLoading } =
    useRequest(postHandeReviewInfo, {
      debounceWait: 300,
  ...
  // 当后端返回的用户评价变更时，同步本地表单初始值
  useEffect(() => {
    setRating(Number(stats?.currentUserReview?.rating || 0));
    setSelectedTags(stats?.currentUserReview?.tags || []);
    setComment(stats?.currentUserReview?.comment || '');
  }, [
    stats?.currentUserReview?.rating,
    stats?.currentUserReview?.tags,
    stats?.currentUserReview?.comment,
  ]);
      ...
      <div key={starValue} className='relative'>
        ...
        {isHalfStar ? (
          <div className='relative'>
            <Star
              className={`w-5 h-5 ${colorClass.replace(
                'fill-',
                'none-'
  ...
  return (
    <div className='p-3 space-y-4'>
            ...
            (tag: string) => {
              ...
              return (
                <button
                  ...
                  style={
                    stats?.defaultTags?.colorTags?.[tag]
                      ? {
      ...
      {errorInfo && (
        <div
          className={
            'text-red-500 text-sm font-medium text-left w-full whitespace-normal transform -translate-y-2 mx-auto'
          }
        >
          {errorInfo}
        </div>
