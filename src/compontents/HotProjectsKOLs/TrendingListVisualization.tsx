...
export function TrendingListVisualization({ items, loading, type }: TrendingListVisualizationProps) {
  ...
  useEffect(() => {
    ...
    const allUsernames = sortedItems.map(item => {
      if ('twitter' in item) {
...
function DiscussionListItem({ item, index, theme, lang, showAvatarRank, userRank, isLoading }: DiscussionListItemProps) {
  // 计算热度指数（转换为更直观的数值）
  ...
  return (
    <div className="px-3 py-2 rounded-md theme-hover transition-colors">
      ...
      >
        <div className="relative rounded-full" style={{ border: '3px solid #60A5FA80' }}>
          <img
          ...
          {showAvatarRank ? (
            <div
              className={`xhunt-avatar-rank-badge ${userRank && userRank > 0 && userRank <= 10000 ? 'high-ranked' : ''} ${isLoading ? 'loading' : ''}`}
              data-theme={theme}
            >
              <span
                className="xhunt-avatar-rank-text"
                dangerouslySetInnerHTML={{ __html: isLoading ? '...' : formatRank(userRank) }}
              ></span>
            </div>
          ) : null}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <p className="font-medium text-sm theme-text-primary truncate">{item.twitter.name}</p>
          ...
          <div className="flex items-center gap-1">
            <p className="text-xs theme-text-secondary truncate">@{item.twitter.username}</p>
            <span className="text-xs theme-text-secondary">•</span>
            <p className="text-xs theme-text-secondary whitespace-nowrap">热度 {heatIndex}</p>
          </div>
        </div>
        {summary && (
          <div className="inline-grid w-full ml-14 -mt-3 mr-2 overflow-ellipsis pointer-events-none">
            <p className="text-xs theme-text-secondary leading-relaxed overflow-ellipsis overflow-hidden whitespace-nowrap">
              {summary}
            </p>
          </div>
        )}
...
function TrendingListItem({ item, index, theme, showAvatarRank, userRank, isLoading, type, lang }: TrendingListItemProps) {
  // 计算热度指数（转换为更直观的数值）
  const heatIndex = Math.round(item.share * 10000);

  // 获取叙事内容（仅项目类型）
  const narrative = type === 'project' && item.twitter.feature?.narrative
    ? (lang === 'zh' ? item.twitter.feature.narrative.cn : item.twitter.feature.narrative.en)
    : null;
