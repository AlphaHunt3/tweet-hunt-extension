  ...
  return (
    <>
      ...
      >
        ...
        {/* Content */}
        {!isLoading && data && (
          <div className={"divide-x theme-border" + data?.tweets && Boolean(data?.tweets?.length) ? "grid grid-cols-2" : ""}>
            {/* Left Column - Tweets */}
                      ...
                      <div className="flex gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1 mb-0.5">
                            <span className="font-medium theme-text-primary text-xs truncate whitespace-nowrap">{tweet.name}</span>
                            {/*<span className="theme-text-secondary text-xs truncate">@{tweet.username}</span>*/}
                            <span className="theme-text-secondary text-xs">·</span>
                            <time className="theme-text-secondary text-xs whitespace-nowrap">{dayjs(tweet.createTime).fromNow()}</time>
                          </div>
