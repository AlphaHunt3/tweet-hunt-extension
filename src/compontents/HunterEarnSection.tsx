...
export function HunterEarnSection({
  ...
  const [savedTab, setSavedTab, { isLoading: isLoadingSavedTab }] =
    useLocalStorage<'hunter' | 'engage'>(
  ...
  const setPortalRef = React.useCallback((node: HTMLDivElement | null) => {
  ...
  const { run: refreshCreatorAuth } = useRequest(
    async () => {
      ...
      if (first?.kolRank !== undefined) {
        await rankService.updateRanks({ [userName]: first.kolRank }, 'web3');
  ...
  React.useEffect(() => {
    const fetchRank = async () => {
      ...
      try {
        const ranks = await rankService.getRanks([userName], 'web3');
  ...
  const hasHunterCampaigns = sortedHunterCampaigns.length > 0;
  const isHunterEmptyState = !showingEngage && !hasHunterCampaigns;
  const isCompactTabs = !hasHunterCampaigns;
  ...
  return (
    <div
      ...
      style={
        {
          '--xhunt-web-bg': 'transparent',
        } as React.CSSProperties
      }
    >
      <div className={`${isHunterEmptyState ? 'px-4 pt-2' : 'px-4 pt-3'} flex items-center justify-between`}>
        <div className='flex items-center gap-2'>
          {showHunter && (
            <button
              type='button'
              className={`relative inline-flex items-center gap-1.5 rounded-lg font-semibold transition-all ${
                isCompactTabs ? 'px-2 py-1 text-[11px]' : 'px-3 py-1.5 text-xs'
              } ${
                !showingEngage
                  ? 'theme-bg-secondary/60 theme-text-secondary border theme-border shadow-none'
                  : 'theme-text-secondary border border-transparent hover:theme-text-primary hover:theme-bg-secondary/60'
              }`}
            ...
            <div className='-mt-1 z-10 relative'>
              <div className='flex flex-col items-center text-center'>
                <div
                  ...
                  style={{
                    background: dark
                      ? 'linear-gradient(135deg, rgba(29,155,240,0.10), rgba(168,85,247,0.10))'
                      : 'linear-gradient(135deg, rgba(29,155,240,0.18), rgba(168,85,247,0.18))',
                  }}
                >
                  <svg
                  ...
                  >
                    <defs node-id='166'>
                      <linearGradient
                        ...
                        y2='714.802'
                      >
                        <stop offset='0.00000009590021' stopColor='#f4f2fb' />
                ...
                {!isAuthorVerifyPending && (
                  <div className='mt-2 flex flex-col items-center gap-1.5'>
                    <button
                      type='button'
                      className={`inline-flex items-center justify-center rounded-full px-4 py-1.5 text-[11px] font-semibold shadow-sm transition-colors bg-transparent border ${
                        dark
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const ret = await postAuthCreator({
                            user_id: userId,
                          });
                          const action =
          ...
          {showingEngage ? (
            ...
            <>
              {sortedHunterCampaigns.length === 0 ? (
                <div className='px-4 pt-1.5 pb-2 flex justify-center'>
                  <div className='inline-flex max-w-full items-center gap-1.5 rounded-full bg-black/[0.03] px-2 py-1 text-[11px] leading-4 theme-text-secondary dark:bg-white/[0.035]'>
                ...
                <>
                  {sortedHunterCampaigns.map((campaignConfig) => (
                    <div
                      data-sort-weight={campaignConfig.sortWeight ?? 0}
                      key={campaignConfig.id}
                      className='px-4 pt-3'
                    >
                  ...
                  <div className='mx-4 mt-3 border-t theme-border' />
                </>
              )}
            </>
          )}
